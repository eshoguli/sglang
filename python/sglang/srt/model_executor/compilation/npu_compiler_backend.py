...
from sglang.srt.model_executor.compilation.debugging import write_to_file
...
class NpuBackend:
    ...
    def split_graph(graph: torch.fx.GraphModule, ops: list[str]) -> tuple[torch.fx.GraphModule, list[SplitItem]]:
        ...
        print(f"NpuBackend::split_graph: {os.getpid()}: subgraph_id={subgraph_id}, len(graphs_for_compilation)={len(graphs_for_compilation)}, counter={counter}", flush=True)
